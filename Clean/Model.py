...
## Diffusion and reaction terms (Run outside class for Numba)
@njit
def f_numba(N, rho, K):
    return rho * N * (1 - N / K)  # logistic growth
...
def build_laplacian_diagonals_avg(m, D, dx):
    ...
    for i in range(1, N - 1):
        ml = 0.5 * (m[i - 1] + m[i])  # left edge-average of M
        mr = 0.5 * (m[i] + m[i + 1])  # right edge-average of M
    ...
    invdx2 = 1.0 / dx**2
    return invdx2 * lower, invdx2 * center, invdx2 * upper  # diagonals for sparse operator


## Solver class with AB2AM2 + Implicit Euler (functional source)
class Dissertation_Func_1D:
    """
    1D tumour–ECM with functional source:
      u_t = (D M(1-M) u_x)_x + rho*u*(1-u/K)
      m_t = alpha*(1 - m) - k*u*m
    IMEX AB2–AM2 for u; implicit Euler for m with u^{n+1}.
    """

    def __init__(self, D=1.0, rho=1.0, K=1.0, k=1.0, alpha=1.0,
        ...
        self.T = T
        self.dt = dt
        self.Nt = int(np.floor(T / dt)) + 1  # include t=0
    ...
    def solve(self):
        ...
        for i in range(2, self.Nt):
            ...
            # Implicit Euler for m with u^{n+1}
    ...
    def track_wavefront_local_interpolation(self, threshold=0.5, band=(0.1, 0.9),
        ...
        def get_spline(method, xg, yg):
            if method == 'cubic':  return CubicSpline(xg, yg)
            if method == 'pchip':  return PchipInterpolator(xg, yg)
            if method == 'akima':  return Akima1DInterpolator(xg, yg)
            if method == 'linear': return interp1d(xg, yg, kind='linear', fill_value="extrapolate")
    ...
    def plot_u_m_with_custom_style(self, 
        ...
        if print_speed and (self.wave_speed is not None):
            print(f"Estimated wave speed c = {self.wave_speed:.6g}")
        ...
        alp_str = _latex_sci(self.alpha, pow10_threshold=100.0)
        ax.set_title(rf"$\lambda = {lam_str},\ \alpha = {alp_str}$", fontsize=20)
        ax.grid(False)
        fig.tight_layout()
        ...
        if save:
            ...
            if filename is None:
                ...
                alp_file = _file_sci(self.alpha, 100.0)
                filename = f"func_{m0_str}_lam{lam_file}_alpha{alp_file}.png"
