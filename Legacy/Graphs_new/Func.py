...
class Dissertation_Func_1D:
    def __init__(self, D=1.0, rho=1.0, K=1.0, k=1.0,
        ...
        """
        Functional source: m_t = alpha*(1 - m) - k*u*m
        """
        # PDE/ODE params
    ...
    def solve(self):
        """
        N: AB2–AM2 (IMEX)
        M: implicit Euler for m_t = alpha*(1 - m) - k*u*m
    ...
    def plot_u_m_with_custom_style(self, t_indices=[0, 500, 750, 1000],
        ...
        plt.text(x_text, 1.42, rf"$\overline{{m}} = {self.m0}$", fontsize=18, ha='left')
        plt.text(x_text, 1.32, rf"$c = {c_str}$", fontsize=18, ha='left')

        # Formatting
        plt.xlabel(r"$x$", fontsize=18)
        plt.ylabel(r"$u(x,t),\, m(x,t)$", fontsize=18)
        plt.ylim([-0.05, 1.55])  # extended to allow m > 1
        plt.yticks([0.0, 0.5, 1.0, 1.5], fontsize=16)
